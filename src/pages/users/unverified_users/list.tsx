...
import { Link } from 'react-router-dom'; 
  ...
  const AllUsersTable: FC = function () {
    ...
    useEffect(() => {
      // Fetch users
      axios
        .get<User[]>('http://209.38.40.216:8000/api/v1/admins/users/not_verified', {
            ...
            <Table.Row key={user.id}>
              ...
              <Table.Cell>
                <div className="flex items-center gap-x-3 whitespace-nowrap">
                  ...
                  <VerifyUserModal userId={user.id} onVerify={() => handleVerifyUser(user.id)} />
                </div>
              </Table.Cell>
            </Table.Row>
          ))}
  ...
  const VerifyUserModal: FC<VerifyUserModalProps> = function ({ userId, onVerify }) {
    const [isOpen, setOpen] = useState(false);
  
    const handleVerify = async () => {
      try {
        await axios.patch(
          `http://209.38.40.216:8000/api/v1/admins/users/verify/${userId}`, 
          {},  
          {
            headers: { Authorization: `Bearer ${Cookies.get('token')}` }
          }
        );
        toast.success('User verifiedsuccessfully');
        onVerify();
        setOpen(false);  
      } catch (error) {
